...
For production, set these environment variables before starting Tomcat � never rely on the
...
1. Click **Add Member**.
2. Fill in the form:
   - Username, Email, Password (≥ 8 characters, must include letters and numbers)
...
first, and the new password must meet the strength policy (≥ 8 characters, including letters
...
> ("Tên đăng nhập hoặc email đã được sử dụng") rather than revealing which one � this prevents
...
| **Password strength policy** | New passwords must be ≥ 8 characters and contain both letters and numbers |
